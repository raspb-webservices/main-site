<script lang="ts">
  ...
  import { projectTypesAiFreestyle } from '$lib/configs/wizard-config';
  let { config, selectProjectType } = $props();
</script>

<div class="step-header">
  <h1>{m['wizard.stepType.titleFirst']()} <span class="inner-text-special">{m['wizard.stepType.titleHighlight']()}</span>  {m['wizard.stepType.titleSecond']()}</h1>
