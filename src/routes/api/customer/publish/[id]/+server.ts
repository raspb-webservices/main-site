...
export const POST: RequestHandler = async ({ params }) => {
  try {
      ...
      return json(
        {
          ...
          details: ['Keine Customer ID in der URL gefunden.']
        },
        { status: 400 }
      );
    }

    console.log('Publishing customer:', customerId);

    // Customer veröffentlichen
    const publishCustomerMutation = `
    ...
    const result = await graphqlRequest(publishCustomerMutation, {
      id: customerId
    });

    if (result.publishCustomer) {
      console.log('Customer erfolgreich veröffentlicht:', result.publishCustomer.id);

      return json({
        success: true,
        data: result.publishCustomer,
        message: 'Customer erfolgreich veröffentlicht'
      });
    } else {
      throw new Error('Customer konnte nicht veröffentlicht werden');
    }
  } catch (error) {
    console.error('Fehler beim Veröffentlichen des Customers:', error);

    let errorMessage = 'Unbekannter Fehler beim Veröffentlichen des Customers';
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('not found')) {
        errorMessage = 'Customer nicht gefunden';
        errorDetails.push('Der angegebene Customer existiert nicht oder wurde bereits gelöscht.');
      } else if (error.message.includes('already published')) {
        errorMessage = 'Customer bereits veröffentlicht';
        errorDetails.push('Der Customer ist bereits veröffentlicht.');

        // Auch bei bereits veröffentlichtem Customer Erfolg zurückgeben
        return json({
          success: true,
          data: { id: params.id },
          message: 'Customer war bereits veröffentlicht'
        });
      } else if (error.message.includes('network') || error.message.includes('fetch')) {
        errorMessage = 'Netzwerkfehler beim Veröffentlichen des Customers';
